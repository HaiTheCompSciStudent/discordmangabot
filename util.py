import re
from exceptions import MangadexError
from contextlib import suppress
import asyncio
import discord
from database import database

test_ref = "https://mangadex.org/title/29329s/"


def split_id(manga_ref):
    m = re.split("/", manga_ref)
    ref_type, manga_id = ("URL", m[4]) if len(m) >= 6 else ("ID", m[0])
    print(manga_id)
    if not re.match("^[0-9]+$", manga_id):
        raise MangadexError(f"Invalid {ref_type} : {manga_ref}")
...
def code_blockify(content, prefix="```", suffix="```"):
...
def _prefix_callable(_, ctx):
    return database.get_guild(ctx.guild.id).prefix
